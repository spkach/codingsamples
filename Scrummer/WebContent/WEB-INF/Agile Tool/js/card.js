$(document).ready(function(){
	...
	$("#moveitButton").click(function() {
		var txtVal = $('#txt1').val();
		$('#tmptxt').val(txtVal);
		var cardd= $.session.get("cardd");
		var type= $.session.get("color");		
		tasksWithCard.push(cardd);
		taskName.push(txtVal);
		taskValue.push(type);
		$.session.set("tasks",tasksWithCard);
		$.session.set("taskName",taskName);
		$.session.set("taskValue",taskValue);
		afterText(txtVal);
		$("#moveitButton").removeClass("btn btn-primary active");
		$("#moveitButton").addClass("btn btn-primary disabled");
	});
